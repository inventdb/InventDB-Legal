"""InventDB Legal — Flask application factory.
...
def create_app() -> Flask:
    ...
    # a time-entry result led with `hours` and pushed `t.matter_id` — the matter
    # number — out of the visible columns entirely, so the same saved analysis
    # showed different columns in this app than in SOAR's Analyze room.
...
def _refuse_occupied_port(host: str, port: int) -> None:
    ...
    That is not a theoretical failure. A sibling InventDB app on 8000 meant this
    frontend proxied ``/api`` straight into the other app's entity registry:
    every module here came back ``404 Unknown entity: matters`` while both
    servers reported themselves healthy. Refusing to start names the collision
    at the moment it happens instead of leaving it to be diagnosed from an empty
    table three layers away.
    """
    ...
    raise SystemExit(
        ...
        f"Stop the other server, or pick another port:  API_PORT=8011"
    )
