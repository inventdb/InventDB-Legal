"""Saved views — a named, reusable filter over one Legal module's list.
...
``baseSql``, ``defaultForType``, ``sort: {col, dir}`` -- rather than this app's own.
...
def _clean_sort(raw: Any) -> dict[str, str] | None:
    ...
    This app's own tables speak ``{field, dir}``; both spellings are accepted so a
...
def _shape(view: dict[str, Any], default_key: str) -> dict[str, Any]:
    """One view, as the Legal front end wants it.
...
def _find(client: InventDBClient, entity: Entity, view_id: str) -> dict[str, Any]:
    ...
    Clients view through the Matters URL -- the id alone would happily
    address either.
    """
    try:
        full = client.get_saved_view(view_id)
...
def update_view(entity_name: str, view_id: str):
    ...
    if "sort" in body:
        ...
        # ORDER BY inside `baseSql` -- because SOAR reads the SQL and this app
