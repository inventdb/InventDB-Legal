"""Generic CRUD API for every legal entity (Flask blueprint).

Routes are mounted under ``/api/<entity>`` for each entity in the registry, e.g.
``/api/matters``, ``/api/time_entries``. Listing uses InventDB SQL (with a
resilient fallback), while create/update/delete use the record endpoints so
InventDB stamps its system fields and preserves history.
"""
...
from ..sqlutil import LIKE_ESCAPE, ident, like_literal, sql_literal
...
def list_records(entity_name: str):
    ...
    if q and entity.search_fields:
        ...
        clauses = [
            f"{ident(f, 'search column')} LIKE {pattern}{LIKE_ESCAPE}"
    ...
    # An entity with no declared search fields still has to honour `q`; there is
    # no column list to build a LIKE over, so that case alone falls back to
    # matching the fetched page.
    if q and not entity.search_fields:
        rows = [r for r in rows if _matches_q(r, q, list(rows[0].keys()) if rows else [])]
        total = len(rows)
