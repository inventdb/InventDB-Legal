...
def test_endpoints_that_return_data_actually_returned_some(api, replay, recordings):
    ...
    for endpoint in ENDPOINTS:
        if endpoint["key"] in {"error-envelope", "matter-delete"}:
            continue
        body = api._open(
            endpoint["method"],
            bind(endpoint["path"], recordings["bindings"]),
...
def test_the_contract_covers_every_response_type_the_frontend_declares():
    ...
    covered_indirectly = {
        "NameValue",
        "CashflowPoint",
        # Row/element types, validated through the reports that carry them:
        # `WipRow` inside `/reports/wip`, `DeadlineRow` inside
        # `/reports/deadlines`, `ArAgingBucket` inside `ArAgingReport`.
        "WipRow",
        "DeadlineRow",
        "ArAgingBucket",
        "WorkflowStep",
