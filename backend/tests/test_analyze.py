...
SSE = (
    ...
    b'event: message\ndata: {"type":"done","content":"46 active invoices."}\n\n'
)
...
def test_stream_is_relayed_verbatim(api, fake):
    ...
    resp = api.post(
        ...
        json={"messages": [{"role": "user", "content": "How many active invoices?"}]},
    )
...
def test_select_reaches_inventdb(api, fake):
    fake.on_sql("FROM legal.matters", rows=[{"_id": "p-1"}])

    body = api.post(
        "/api/analyze/sql", json={"sql": "SELECT _id FROM legal.matters LIMIT 1"}
    ).get_json()

    assert body["rows"] == [{"_id": "p-1"}]
    assert fake.only_sql() == "SELECT _id FROM legal.matters LIMIT 1"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "UPDATE legal.invoices SET rent = 0",
        "DROP TABLE legal.costs_and_disbursements",
        "INSERT INTO legal.costs_and_disbursements (description) VALUES ('x')",
        "  \n TRUNCATE legal.costs_and_disbursements",
    ],
)
...
def test_a_second_statement_is_refused(api, fake):
    ...
    resp = api.post(
        "/api/analyze/sql", json={"sql": "SELECT 1; DROP TABLE legal.costs_and_disbursements"}
    )
...
def test_change_set_is_forced_into_this_namespace(api, fake):
    ...
    assert step["typeName"] == "costs_and_disbursements"
...
def test_change_set_results_are_unwrapped(api, fake):
    ...
    body = api.post(
        "/api/analyze/change-set/apply",
        json={"steps": [{"op": "insert", "typeName": "costs_and_disbursements", "fields": {}}]},
    ).get_json()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"op": "insert", "typeName": "legal.costs_and_disbursements"},  # a dotted escape attempt
        {"op": "insert", "typeName": ""},
        {"op": "wipe", "typeName": "costs_and_disbursements"},  # not an operation we forward
    ],
)
...
def test_create_record_strips_internal_fields(api, fake):
    fake.on("POST", f"/db/{NAMESPACE}/costs_and_disbursements", {"id": "v-1"})

    body = api.post(
        "/api/analyze/records/costs_and_disbursements",
        json={"description": "Blue Ridge", "_id": "spoofed", "_createdBy": "someone"},
    ).get_json()

    assert body == {"ok": True, "recordId": "v-1", "type": "costs_and_disbursements"}
    assert fake.last_call("POST", f"/db/{NAMESPACE}/costs_and_disbursements").body == {
...
def test_update_merges_over_the_stored_document(api, fake):
    ...
    # replaces or merges.
    fake.on_record("costs_and_disbursements", "v-1", {"_id": "v-1", "description": "Old", "trade": "Roofing"})
    fake.on("PUT", f"/db/{NAMESPACE}/costs_and_disbursements", {"ok": True})

    api.put("/api/analyze/records/costs_and_disbursements/v-1", json={"description": "New"})

    sent = fake.last_call("PUT", f"/db/{NAMESPACE}/costs_and_disbursements").body
    assert sent == {"_id": "v-1", "description": "New", "trade": "Roofing"}
...
def test_staging_a_file_forwards_it_as_multipart(api, fake):
    fake.on("POST", "/ai/uploads/stage", {"pending_id": "pend-1", "filename": "invoice.pdf"})

    body = api.post(
        "/api/analyze/uploads/stage",
        data={"file": (io.BytesIO(b"%PDF-1.7 invoice"), "invoice.pdf")},
