...
1,171 tests over the Flask API, the InventDB client and the SQL helpers. No
network, no live instance, no database — the whole suite runs in about thirty
seconds.
...
def test_arbitrary_query_params_become_equality_filters(api, fake):
    api.get("/api/matters", query_string={"client_name": "Pineda, Carmen"})
    assert fake.sql_log[0] == (
        "SELECT * FROM legal.matters WHERE client_name = 'Pineda, Carmen' "
...
7. **`reports.py` hardcodes `NS = "legal"`** (`test_reports.py`) instead of the
   client's configured namespace, unlike every other router. Deploying against a
   differently-named namespace leaves those endpoints querying a namespace that
...
9. **A `GROUP BY` collapses when the `WHERE` holds a function call**
   (`test_dashboard.py`, `test_reports.py`). This is InventDB's behaviour, not
   the app's: `WHERE lower(col) = 'x'` alongside `GROUP BY y, m` returns a
   single row carrying the grand total. The aggregates group on the column and
   split in Python instead, and both suites pin that shape so nobody
   "simplifies" it back.
...
- `ident`, `_safe_ident` and `_safe_id` used `re.match` with a `$`-anchored
  ...
  the allow-lists accepted `"matters\n"`. Now `re.fullmatch`.
