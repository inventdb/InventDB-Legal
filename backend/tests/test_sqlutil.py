...
@pytest.mark.parametrize(
    ...
    [
        ...
        "a\tb",
        "a b OR 1=1",
        "1; DROP TABLE matters",
        "matters; DROP TABLE matters; --",
        "matters WHERE 1=1",
        "matters\x00",
    ],
)
...
@pytest.mark.parametrize("value", ["matters\n", "matters\r\n", "matters\n\n"])
def test_ident_rejects_trailing_newlines(value):
    ...
    final newline, so the natural-looking ``^ident$`` accepts ``"matters\\n"``.
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "x' AND (SELECT COUNT(*) FROM legal.timekeepers) > 0 --",
...
# ===========================================================================
# like_literal() — the free-text search predicate
# ===========================================================================
...
def test_like_literal_escapes_like_wildcards(value, expected):
    """A search for "100%" looks for that text, not for every row.

    ``%`` and ``_`` are wildcards to the engine, so a term containing either
    would otherwise widen the search instead of narrowing it — the user types
    more and gets more back. Escaped here, and declared as escaped by pairing
    every generated comparison with :data:`LIKE_ESCAPE`.
    """
    assert like_literal(value) == expected


def test_like_literal_escapes_the_escape_character_itself():
    """Otherwise a literal backslash would escape whatever followed it."""
    assert like_literal("a" + "\\" + "b") == r"'%a\\b%'"
