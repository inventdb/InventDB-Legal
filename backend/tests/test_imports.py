...
def test_a_batch_goes_to_the_bulk_endpoint(api, fake: FakeInventDB):
    resp = api.post(
        "/api/import/matters",
        json={"rows": [{"matter_caption": "44 Cedar Lane"}, {"matter_caption": "12 Oak Road"}]},
    )
    assert resp.status_code == 200
    assert resp.get_json() == {"ok": True, "created": 2, "type": "matters"}

    call = fake.last_call("POST")
    assert "/matters/bulk" in call.path
    assert [r["matter_caption"] for r in call.body] == ["44 Cedar Lane", "12 Oak Road"]


def test_the_namespace_is_pinned_server_side(api, fake: FakeInventDB):
    """The caller names a type, never a database."""
    api.post("/api/import/matters", json={"rows": [{"matter_caption": "x"}]})
    assert "/legal/matters/bulk" in fake.last_call("POST").path
...
def test_a_dangerous_type_name_never_reaches_inventdb(api, fake: FakeInventDB):
    ...
    resp = api.post("/api/import/legal.matters", json={"rows": [{"a": 1}]})
...
def test_inventdb_keys_are_stripped(api, fake: FakeInventDB):
    ...
    api.post(
        "/api/import/matters",
        json={"rows": [{"_id": "rec-1", "_createdAt": "x", "matter_caption": "44 Cedar Lane"}]},
    )
    written = fake.last_call("POST").body[0]
    assert written == {"matter_caption": "44 Cedar Lane"}
...
def test_typed_values_are_relayed_unchanged(api, fake: FakeInventDB):
    ...
    api.post(
        "/api/import/matters",
        json={"rows": [{"invoice_total": 2100, "w_9_on_file": True, "lease_end": "2026-06-30"}]},
    )
...
def test_an_upstream_failure_is_reported_rather_than_swallowed(api, fake: FakeInventDB):
    ...
    resp = api.post("/api/import/matters", json={"rows": [{"matter_caption": "x"}]})
    assert resp.status_code >= 400
...
def test_a_malformed_body_is_a_client_error(api, body, fake: FakeInventDB):
    resp = api.post("/api/import/matters", json=body)
    assert resp.status_code == 400
    assert fake.calls_to("POST") == []
...
def test_rows_that_hold_only_underscore_keys_are_refused(api, fake: FakeInventDB):
    ...
    resp = api.post("/api/import/matters", json={"rows": [{"_id": "rec-1"}]})
    assert resp.status_code == 400
    assert fake.calls_to("POST") == []
...
def test_a_batch_over_the_ceiling_is_refused(api, fake: FakeInventDB):
    resp = api.post(
        "/api/import/matters", json={"rows": [{"matter_caption": "x"} for _ in range(5001)]}
    )
    assert resp.status_code == 400
    assert fake.calls_to("POST") == []
...
def test_importing_requires_a_token(api):
    resp = api.post("/api/import/matters", json={"rows": [{"matter_caption": "x"}]}, token=None)
