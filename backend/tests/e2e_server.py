...
The stand-in is deliberately small. It answers what the app actually sends —
...
test here should assert on InventDB's behaviour; the point is the app's own
...
os.environ.setdefault("INVENTDB_NAMESPACE", "legal")
os.environ.setdefault("INVENTDB_APP", "legal")
os.environ.setdefault("CORS_ORIGINS", "http://localhost:5174")
os.environ.setdefault("LEGAL_STATE_FILE", os.path.join(os.path.dirname(__file__), "_e2e_state.json"))
...
SEED: dict[str, list[dict[str, Any]]] = {
    "matters": [
        {
            "_id": "p1", "matter_id": "P-1001", "matter_caption": "44 Cedar Lane",
            "client_name": "Richmond", "state": "VA", "region": "Central VA",
            "type": "Townhouse", "status": "Occupied", "beds": 3, "baths": 2,
            "invoice_total": 2100, "client_id": "O-1001",
        },
        {
            "_id": "p2", "matter_id": "P-1002", "matter_caption": "12 Oak Road",
            "client_name": "Norfolk", "state": "VA", "region": "Coastal VA",
            "type": "Condo", "status": "Vacant", "beds": 2, "baths": 1,
            "invoice_total": 1650, "client_id": "O-1002",
        },
    ],
    "clients": [
        {"_id": "o1", "client_id": "O-1001", "name": "Harbourline Holdings",
         "type": "LLC", "email": "meera@harbourline.example", "mgmt_fee": 8},
        {"_id": "o2", "client_id": "O-1002", "name": "Patterson Family Trust",
         "type": "Trust", "email": "eleanor@example.com", "mgmt_fee": 10},
    ],
    "invoices": [
        {"_id": "l1", "invoice_no": "L-7001", "matter_id": "P-1001",
         "timekeeper_id": "T-9001", "status": "Active", "invoice_total": 2100,
         "lease_end": "2026-06-30"},
    ],
    "time_entries": [
        {"_id": "w1", "wo": "WO-1", "matter_id": "P-1001", "category": "plumbing",
...
def transport(method: str, url: str, **kw: Any) -> _Response:
    ...
    if path == "/api/saved-views/generate-layout":
        return _Response(200, {
            ...
            "sql": f"SELECT * FROM legal.matters",
        })
