...
def test_a_structured_detail_survives_into_the_response(api, fake):
    ...
    fake.on("POST", "/db/legal/matters", {"error": {"beds": "must be an integer"}}, status=422)

    resp = api.post("/api/matters", json={"beds": "three"})
...
def test_without_a_bundled_frontend_the_root_describes_the_api(client):
    ...
    assert body["name"] == "InventDB Legal API"
...
def test_a_built_frontend_is_served_from_the_root(tmp_path, monkeypatch, fake):
    dist = tmp_path / "dist"
    dist.mkdir()
    (dist / "index.html").write_text("<!doctype html><title>Legal</title>", encoding="utf-8")
    (dist / "app.js").write_text("console.log(1)", encoding="utf-8")
    monkeypatch.setenv("FRONTEND_DIST", str(dist))

    app_client = create_app().test_client()

    assert b"<title>Legal</title>" in app_client.get("/").data
...
def test_client_side_routes_fall_back_to_the_index(tmp_path, monkeypatch, fake):
    """A hard refresh on /matters/abc must serve the SPA shell, not a 404 —
    ...
    assert b"<title>Legal</title>" in app_client.get("/matters/abc").data
    assert b"<title>Legal</title>" in app_client.get("/reports").data
...
def test_a_port_someone_else_is_serving_is_refused():
    ...
    two apps. A sibling InventDB app on 8000 is how this frontend ended up
    proxying into the wrong entity registry, with every module answering
    ``404 Unknown entity``.
    """
...
def test_the_default_port_does_not_collide_with_the_sibling_app():
    """8000 belongs to the other InventDB app on a machine running both."""
