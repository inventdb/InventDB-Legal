"""``app.routers.workflows`` — this app's proxy onto InventDB's workflow engine.
...
WORKFLOWS = [{"_id": "wf-1", "name": "Weekly Invoice Renewals Due"}]
RUNS = [{"_id": "run-1", "status": "success"}]

PLAN = [{"idx": 0, "kind": "sql_query", "label": "Find invoices", "narration": "…", "sql": "SELECT 1"}]
...
def test_a_new_workflow_is_sandboxed_unless_the_author_says_otherwise(api, fake):
    """A workflow that emails clients should rehearse before it can reach
