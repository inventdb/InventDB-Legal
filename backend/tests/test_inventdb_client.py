...
@pytest.mark.parametrize("value", ["legal", "_x", "time_entries", "A9"])
...
def test_an_empty_body_becomes_none(db, fake):
    fake.on("DELETE", "/db/legal/matters/abc", empty=True)
    assert db.delete_record("matters", "abc") is None
...
def test_me_scopes_the_lookup_to_this_app(db, fake):
    db.me()
    assert fake.last_call("GET", "/api/auth/me").params == {"app": "legal"}
...
def test_list_types_requests_metadata_for_the_namespace(db, fake):
    db.list_types()
    call = fake.last_call("GET", "/api/namespaces/legal/types")
    assert call.params == {"metadata": "true"}


def test_relationships_is_namespace_scoped(db, fake):
    db.relationships()
    assert fake.last_call("GET", "/api/relationships/legal")
...
def test_record_paths_are_namespace_and_type_qualified(db, fake):
    db.get_record("matters", "abc")
    assert fake.calls[-1].path == "/db/legal/matters/abc"


@pytest.mark.parametrize(
    "operation,method,path",
    [
        (lambda c: c.create_record("matters", {"a": 1}), "POST", "/db/legal/matters"),
        (lambda c: c.update_record("matters", {"a": 1}), "PUT", "/db/legal/matters"),
        (lambda c: c.delete_record("matters", "abc"), "DELETE", "/db/legal/matters/abc"),
    ],
)
...
@pytest.mark.parametrize("type_name", ["../secrets", "a b", "legal.users", "", "1x"])
...
def test_record_ids_should_be_validated_like_template_ids(db, fake, record_id, leak):
    with pytest.raises(ApiError):
        db.get_record("matters", record_id)


def test_bulk_insert_targets_the_bulk_endpoint(db, fake):
    db.bulk_insert("matters", [{"a": 1}, {"a": 2}])
    call = fake.last_call("POST", "/api/legal/matters/bulk")
    assert call.params == {"disableIndexing": "false"}
    assert call.body == [{"a": 1}, {"a": 2}]


def test_delete_by_filter_wraps_the_filter(db, fake):
    db.delete_by_filter("matters", {"client_name": "Mumbai"})
    assert fake.last_call("POST", "/api/legal/matters/delete").body == {
        "filter": {"client_name": "Mumbai"}
    }
...
def test_truncate_requires_the_confirmation_token(db, fake):
    ...
    db.truncate("matters")
    assert fake.last_call("POST", "/api/legal/matters/deleteAll").body == {
        "confirm": "CONFIRM"
    }


def test_filter_records_posts_to_the_query_endpoint(db, fake):
    db.filter_records("matters", {"where": {"client_name": "Mumbai"}})
    assert fake.last_call("POST", "/query/legal/matters/filter").body == {
        "where": {"client_name": "Mumbai"}
    }
...
def test_query_rows_swallows_sql_errors(db, fake):
    ...
    namespace usable."""
    fake.on_sql(status=500, payload={"error": "no such table: legal.matters"})
    assert db.query_rows("SELECT * FROM legal.matters") == []
...
def test_render_asks_for_real_charts_not_the_email_safe_fallback(db, fake):
    ...
    db.render_report_template("tpl-1", {"client_id": "O-1"})

    call = fake.last_call("POST", "/api/report-templates/tpl-1/render")
    assert call.body == {"params": {"client_id": "O-1"}, "email_safe_charts": False}
