...
Three matters are worth protecting, and they are what these cover.
...
RESULTS = [{"_id": "att-1", "filename": "invoice.pdf", "record_type": "invoices", "record_id": "lea-1"}]
...
def upload(name: str = "invoice.pdf", content: bytes = b"%PDF-1.4 invoice", ctype: str = "application/pdf"):
...
def test_a_search_is_pinned_to_this_apps_namespace(api, fake):
    """The one matter the whole room rests on."""
    api.post("/api/files/search", json={"query": "invoice"})

    sent = fake.last_call("POST", "/attach/_search").body
    assert sent["namespaces"] == ["legal"]
...
def test_a_selected_type_scopes_the_search(api, fake):
    api.post("/api/files/search", json={"query": "*", "types": ["invoices"], "folder": "2026"})

    sent = fake.last_call("POST", "/attach/_search").body
    assert sent["types"] == ["invoices"]
...
def test_an_empty_folder_is_forwarded_because_it_means_the_type_root(api, fake):
    ...
    api.post("/api/files/search", json={"query": "*", "types": ["invoices"], "folder": ""})
...
@pytest.mark.parametrize("bad", [{"types": "invoices"}, {"types": [1]}, {"limit": "10"}, {"folder": 7}])
...
def test_deleting_a_folder_names_the_namespace_type_and_folder(api, fake):
    ...
    body = api.post("/api/files/bulk-delete", json={"type": "invoices", "folder": "2026"}).get_json()

    assert body == {"deleted": 15, "skipped": 0, "remaining": None}
    sent = fake.last_call("POST", "/attach/_bulk_delete").body
    assert sent["namespace"] == "legal"
    assert sent["type"] == "invoices"
    assert sent["folder"] == "2026"
...
def test_deleting_a_whole_type_omits_the_folder(api, fake):
    ...
    api.post("/api/files/bulk-delete", json={"type": "invoices"})
...
def test_an_unbounded_delete_batch_is_clamped(api, fake):
    ...
    api.post("/api/files/bulk-delete", json={"type": "invoices", "limit": 10_000})
...
def test_remaining_rides_back_when_inventdb_reports_it(api, fake):
    ...
    assert api.post("/api/files/bulk-delete", json={"type": "invoices"}).get_json() == {
        "deleted": 15,
        "skipped": 2,
        "remaining": 40,
    }
...
def test_uploading_sends_the_bytes_and_the_filename(api, fake):
    fake.on("POST", "/attach/legal/invoices/lea-1", {"ok": True, "data": {"attachment_id": "att-9"}})

    resp = api.post(
        "/api/files/invoices/lea-1",
        data=upload(),
    ...
    filename, content, ctype = fake.last_call("POST", "/attach/legal/invoices/lea-1").upload
    assert filename == "invoice.pdf"
    assert content == b"%PDF-1.4 invoice"
...
def test_uploading_into_a_folder_carries_the_path(api, fake):
    ...
    assert fake.last_call("POST", "/attach/legal/invoices/lea-1").data == {"folder_path": "2026/signed"}


def test_an_upload_with_no_folder_sends_none(api, fake):
    api.post("/api/files/invoices/lea-1", data=upload(), content_type="multipart/form-data")
    assert fake.last_call("POST", "/attach/legal/invoices/lea-1").data is None
...
def test_an_unusable_upload_is_refused_before_it_reaches_inventdb(api, fake, payload):
    resp = api.post("/api/files/invoices/lea-1", data=payload, content_type="multipart/form-data")
    assert resp.status_code == 400
    assert fake.calls == []
...
def test_an_oversized_upload_is_refused(api, fake):
    ...
    resp = api.post("/api/files/invoices/lea-1", data=big, content_type="multipart/form-data")
    assert resp.status_code == 413
    assert fake.calls == []


def test_listing_a_records_files_normalises_to_an_array(api, fake):
    fake.on("GET", "/attach/legal/invoices/lea-1", {"ok": True, "data": {"attachments": RESULTS}})
    assert api.get("/api/files/invoices/lea-1").get_json() == {"files": RESULTS}


def test_deleting_one_file_addresses_it_by_its_home(api, fake):
    api.delete("/api/files/invoices/lea-1/att-1")
    assert fake.calls[-1].path == "/attach/legal/invoices/lea-1/att-1"
...
def test_extracted_text_is_returned_as_an_object_even_when_sent_as_a_string(api, fake):
    ...
    `.text` either way."""
    fake.on("GET", "/attach/legal/invoices/lea-1/att-1/text", {"ok": True, "data": "Invoice agreement…"})
    assert api.get("/api/files/invoices/lea-1/att-1/text").get_json() == {"text": "Invoice agreement…"}
...
def test_a_download_streams_the_bytes_back_unchanged(api, fake):
    fake.on("GET", "/attach/legal/invoices/lea-1/att-1/download", raw=b"%PDF-1.4 bytes")

    resp = api.get("/api/files/invoices/lea-1/att-1/download?filename=invoice.pdf")
...
def test_a_download_offers_to_save_while_a_preview_displays(api, fake):
    ...
    fake.on("GET", "/attach/legal/invoices/lea-1/att-1/download", raw=b"x")

    save = api.get("/api/files/invoices/lea-1/att-1/download?filename=a.pdf")
    show = api.get("/api/files/invoices/lea-1/att-1/preview")
...
def test_a_thumbnail_asks_for_the_size_it_wants(api, fake):
    fake.on("GET", "/attach/legal/invoices/lea-1/att-1/thumbnail", raw=b"\xff\xd8jpeg")

    resp = api.get("/api/files/invoices/lea-1/att-1/thumbnail?size=128")
...
def test_an_absurd_thumbnail_size_is_clamped(api, fake):
    api.get("/api/files/invoices/lea-1/att-1/thumbnail?size=99999")
    assert fake.calls[-1].params == {"size": 1024}


def test_a_missing_file_keeps_its_404(api, fake):
    fake.on("GET", "/attach/legal/invoices/lea-1/att-x/download", {"error": "Not found"}, status=404)
    assert api.get("/api/files/invoices/lea-1/att-x/download").status_code == 404
...
# question a document exists to answer: which invoice is this?
# ===========================================================================
...
def test_attaching_moves_the_file_onto_the_record(api, fake):
    fake.on(
        "POST",
        "/attach/legal/_relink",
        {"ok": True, "data": {"mode": "move", "parents": [{"namespace": "legal", "typeName": "invoices", "recordId": "lea-1"}]}},
    )

    body = api.post(
        "/api/files/attach",
        json={"attachment_id": "att-1", "type": "invoices", "record_id": "lea-1"},
    ).get_json()

    assert body["mode"] == "move"
    sent = fake.last_call("POST", "/attach/legal/_relink").body
    assert sent["attachment_id"] == "att-1"
    assert sent["target"] == {"namespace": "legal", "typeName": "invoices", "recordId": "lea-1"}


def test_moving_is_the_default_because_a_vault_file_has_no_real_parent(api, fake):
    api.post("/api/files/attach", json={"attachment_id": "att-1", "type": "invoices", "record_id": "lea-1"})
    assert fake.last_call("POST", "/attach/legal/_relink").body["mode"] == "move"
...
def test_a_file_can_belong_to_two_records(api, fake):
    ...
    api.post(
        "/api/files/attach",
        json={"attachment_id": "att-1", "type": "time_entries", "record_id": "wo-2", "mode": "copy"},
    )
    assert fake.last_call("POST", "/attach/legal/_relink").body["mode"] == "copy"
...
def test_attaching_cannot_reach_another_namespace(api, fake):
    ...
    sent = fake.last_call("POST", "/attach/legal/_relink").body
    assert sent["target"]["namespace"] == "legal"
    assert sent["target"]["typeName"] == "invoices"


@pytest.mark.parametrize(
    "body,message",
    [
        ({"type": "invoices", "record_id": "lea-1"}, "attachment_id is required"),
        ({"attachment_id": "att-1", "record_id": "lea-1"}, "type is required"),
        ({"attachment_id": "att-1", "type": "invoices"}, "record_id is required"),
        ({"attachment_id": "att-1", "type": "invoices", "record_id": "  "}, "record_id is required"),
    ],
)
...
def test_an_unknown_attach_mode_is_refused(api, fake):
    resp = api.post(
        "/api/files/attach",
        json={"attachment_id": "att-1", "type": "invoices", "record_id": "lea-1", "mode": "teleport"},
    )
    assert resp.status_code == 400
    assert fake.calls == []
...
def test_version_history_is_normalised_to_an_array(api, fake):
    fake.on(
        "GET",
        "/attach/legal/invoices/lea-1/att-1/versions",
        {"ok": True, "data": {"versions": [{"version": 1}]}},
    )
    assert api.get("/api/files/invoices/lea-1/att-1/versions").get_json() == {
        "versions": [{"version": 1}]
    }


def test_uploading_again_adds_a_version_rather_than_replacing(api, fake):
    fake.on("POST", "/attach/legal/invoices/lea-1/att-1/versions", {"ok": True, "data": {"version": 2}})

    resp = api.post(
        "/api/files/invoices/lea-1/att-1/versions",
        data=upload("invoice-v2.pdf"),
        content_type="multipart/form-data",
    )

    assert resp.status_code == 201
    assert fake.last_call("POST", "/attach/legal/invoices/lea-1/att-1/versions").upload[0] == "invoice-v2.pdf"


def test_restoring_makes_an_earlier_version_current(api, fake):
    api.post("/api/files/invoices/lea-1/att-1/versions/2/restore", json={})
    assert fake.calls[-1].path == "/attach/legal/invoices/lea-1/att-1/versions/2/restore"


def test_a_non_numeric_version_is_not_a_route(api, fake):
    """`<int:version>` keeps a crafted segment out of the upstream URL."""
    assert api.post("/api/files/invoices/lea-1/att-1/versions/x/restore", json={}).status_code == 404
    assert fake.calls == []


def test_an_old_version_can_be_downloaded_without_restoring_it(api, fake):
    fake.on("GET", "/attach/legal/invoices/lea-1/att-1/versions/1/download", raw=b"old")
    assert api.get("/api/files/invoices/lea-1/att-1/versions/1/download").get_data() == b"old"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("get", "/api/files/invoices/lea-1"),
        ("get", "/api/files/invoices/lea-1/att-1"),
        ("delete", "/api/files/invoices/lea-1/att-1"),
        ("get", "/api/files/invoices/lea-1/att-1/text"),
        ("get", "/api/files/invoices/lea-1/att-1/download"),
        ("get", "/api/files/invoices/lea-1/att-1/preview"),
        ("get", "/api/files/invoices/lea-1/att-1/thumbnail"),
        ("get", "/api/files/invoices/lea-1/att-1/versions"),
        ("post", "/api/files/invoices/lea-1/att-1/versions/1/restore"),
    ],
)
def test_no_file_route_answers_without_authentication(api, fake, method, path):
    resp = getattr(api, method)(
        path,
        json={"type": "invoices", "query": "*", "attachment_id": "att-1", "record_id": "lea-1"},
        token=None,
    )
