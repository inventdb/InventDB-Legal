...
def test_list_returns_only_this_modules_views(api, fake):
    _seed(
        fake,
        [
            _stored("v1", "Vacant", entity="matters"),
            _stored("v2", "Expiring soon", entity="invoices"),
            _stored("v3", "By client_name", entity="matters"),
        ],
    )

    resp = api.get("/api/views/matters")

    assert resp.status_code == 200
    names = [v["name"] for v in resp.get_json()["items"]]
    assert names == ["By client_name", "Vacant"]  # alphabetical, invoices excluded


def test_list_ignores_a_same_named_type_in_another_namespace(api, fake):
    _seed(fake, [_stored("v1", "Elsewhere", entity="matters", namespace="other")])
...
def test_the_default_flag_is_reported_per_module(api, fake):
    _seed(
        fake,
        [
            _stored("v1", "Vacant", default_for=f"{NAMESPACE}.matters"),
            _stored("v2", "By client_name"),
        ],
    )

    items = api.get("/api/views/matters").get_json()["items"]

    by_name = {v["name"]: v for v in items}
    assert by_name["Vacant"]["is_default"] is True
    assert by_name["By client_name"]["is_default"] is False
...
def test_create_composes_base_sql_and_stores_the_live_filter(api, fake):
    ...
    resp = api.post(
        "/api/views/matters",
        json={"name": "Vacant", "search": "vacant", "sort": {"col": "client_name", "dir": "desc"}},
    )

    assert resp.status_code == 201
    sent = fake.last_call("POST", "/api/saved-views").body
    assert sent["baseSql"] == f"SELECT * FROM {NAMESPACE}.matters ORDER BY client_name DESC"
    assert sent["search"] == "vacant"
    assert sent["sort"] == {"col": "client_name", "dir": "desc"}
...
def test_create_accepts_the_tables_own_sort_spelling(api, fake):
    ...
    api.post(
        "/api/views/matters",
        json={"name": "By client_name", "sort": {"field": "client_name", "dir": "asc"}},
    )

    # `field` is what this app's table emits; `col` is what SOAR reads. Only one
...
def test_base_sql_omits_pagination(api, fake):
    ...
    api.post("/api/views/matters", json={"name": "Vacant"})
...
def test_creating_a_default_view_takes_a_second_call(api, fake):
    ...
    resp = api.post("/api/views/matters", json={"name": "Vacant", "is_default": True})

    assert resp.status_code == 201
    assert fake.last_call("PUT", "/api/saved-views/new-1").body == {
        "defaultForType": f"{NAMESPACE}.matters"
    }


def test_a_nameless_view_is_refused(api, fake):
    resp = api.post("/api/views/matters", json={"name": "   "})
...
def test_an_over_long_name_is_refused(api, fake):
    resp = api.post("/api/views/matters", json={"name": "x" * 200})
...
def test_a_sort_column_that_is_not_an_identifier_is_refused(api, fake):
    resp = api.post(
        "/api/views/matters",
        json={"name": "Injected", "sort": {"col": "client_name; DROP TABLE legal.matters"}},
    )
...
def test_a_create_that_returns_no_id_is_reported(api, fake):
    ...
    resp = api.post("/api/views/matters", json={"name": "Vacant"})
...
def test_rename_patches_only_the_name(api, fake):
    ...
    resp = api.put("/api/views/matters/v1", json={"name": "Vacant units"})
...
def test_changing_the_sort_moves_base_sql_with_it(api, fake):
    ...
    api.put("/api/views/matters/v1", json={"sort": {"col": "client_name", "dir": "desc"}})

    body = fake.last_call("PUT", "/api/saved-views/v1").body
    assert body["sort"] == {"col": "client_name", "dir": "desc"}
    assert body["baseSql"] == f"SELECT * FROM {NAMESPACE}.matters ORDER BY client_name DESC"


def test_clearing_the_sort_drops_the_order_by(api, fake):
    _seed(fake, [_stored("v1", "Vacant", sort={"col": "client_name", "dir": "asc"})])
    fake.on("PUT", "/api/saved-views/v1", {"ok": True})

    api.put("/api/views/matters/v1", json={"sort": None})

    body = fake.last_call("PUT", "/api/saved-views/v1").body
    assert body["sort"] is None
    assert body["baseSql"] == f"SELECT * FROM {NAMESPACE}.matters"


def test_an_empty_patch_writes_nothing(api, fake):
    _seed(fake, [_stored("v1", "Vacant")])

    resp = api.put("/api/views/matters/v1", json={})
...
def test_a_view_belonging_to_another_module_is_not_reachable(api, fake):
    _seed(fake, [_stored("v1", "Expiring", entity="invoices")])

    resp = api.put("/api/views/matters/v1", json={"name": "Hijacked"})
...
def test_delete_removes_the_view(api, fake):
    ...
    resp = api.delete("/api/views/matters/v1")
...
def test_promoting_a_view_clears_the_previous_default(api, fake):
    ...
    resp = api.put("/api/views/matters/v2/default")
...
def test_promoting_the_current_default_is_a_no_op(api, fake):
    _seed(fake, [_stored("v1", "Vacant", default_for=f"{NAMESPACE}.matters")])

    resp = api.put("/api/views/matters/v1/default")
...
def test_clearing_the_default_leaves_the_views_alone(api, fake):
    ...
    resp = api.delete("/api/views/matters/default")
...
def test_another_modules_default_is_never_touched(api, fake):
    _seed(
        ...
        [
            _stored("v1", "Vacant"),
            _stored("v2", "Expiring", entity="invoices", default_for=f"{NAMESPACE}.invoices"),
        ],
    )
    fake.on("PUT", "/api/saved-views/v1", {"ok": True})

    api.put("/api/views/matters/v1/default")
...
@pytest.mark.parametrize(
    "method,path",
    [
        ("GET", "/api/views/matters"),
        ("POST", "/api/views/matters"),
        ("PUT", "/api/views/matters/v1"),
        ("DELETE", "/api/views/matters/v1"),
        ("PUT", "/api/views/matters/v1/default"),
        ("DELETE", "/api/views/matters/default"),
    ],
)
...
def test_every_module_has_a_view_list(api, fake):
    """Views are a matter of the module registry, not a per-page feature."""
...
def test_design_generates_stores_and_proves_the_layout_renders(api, fake):
    _design_ok(fake, sql=f"SELECT * FROM {NAMESPACE}.matters ORDER BY client_name ASC")

    resp = api.post(
        "/api/views/matters/design", json={"instruction": "a card per matter"}
    )
    ...
    assert body["sql"] == f"SELECT * FROM {NAMESPACE}.matters ORDER BY client_name ASC"

    sent = fake.last_call("POST", "/api/saved-views/generate-layout").body
    assert sent["baseSql"] == f"SELECT * FROM {NAMESPACE}.matters"
    assert sent["namespace"] == NAMESPACE
    ...
    assert sent["instruction"].startswith("a card per matter")
...
def test_design_without_an_instruction_is_refused(api, fake):
    resp = api.post("/api/views/matters/design", json={"instruction": "  "})
...
def test_editing_a_design_amends_the_template_source(api, fake):
    ...
    resp = api.post(
        "/api/views/matters/design",
        json={
            "instruction": "make the rent bold",
            "template_id": "tpl-1",
            "history": ["a card per matter"],
        },
    )
    ...
    assert sent["history"] == ["a card per matter"]
...
def test_a_styling_only_edit_returns_no_query(api, fake):
    ...
    resp = api.post(
        "/api/views/matters/design", json={"instruction": "make the badges green"}
    )
...
def test_a_query_that_wandered_to_another_module_is_ignored(api, fake):
    _design_ok(fake, sql=f"SELECT * FROM {NAMESPACE}.invoices")

    resp = api.post("/api/views/matters/design", json={"instruction": "cards"})
...
@pytest.mark.parametrize(
    "sql",
    ["DELETE FROM legal.matters", "UPDATE legal.matters SET client_name = 'x'"],
)
def test_a_starting_query_that_is_not_a_read_is_refused(api, fake, sql):
    resp = api.post(
        "/api/views/matters/design", json={"instruction": "cards", "base_sql": sql}
    )
...
def test_saving_a_designed_view_points_it_at_the_layout(api, fake):
    ...
    resp = api.post(
        ...
        json={
            "name": "Matter cards",
            "template_id": "tpl-1",
            "base_sql": f"SELECT * FROM {NAMESPACE}.matters ORDER BY client_name ASC",
        },
    )
    ...
    assert view["baseSql"] == f"SELECT * FROM {NAMESPACE}.matters ORDER BY client_name ASC"
...
def test_saving_names_the_layout_after_the_view(api, fake):
    ...
    api.post(
        "/api/views/matters", json={"name": "Matter cards", "template_id": "tpl-1"}
    )
...
def test_a_designed_views_query_must_still_read_this_module(api, fake):
    resp = api.post(
        ...
        json={
            "name": "Wrong",
            "template_id": "tpl-1",
            "base_sql": f"SELECT * FROM {NAMESPACE}.invoices",
        },
    )
...
def test_a_custom_view_is_listed_with_its_template(api, fake):
    stored = _stored("v1", "Matter cards")
    stored["customTemplateId"] = "tpl-1"
    _seed(fake, [stored])

    item = api.get("/api/views/matters").get_json()["items"][0]
...
@pytest.mark.parametrize(
    "method,path",
    [
        ("POST", "/api/views/matters/design"),
        ("POST", "/api/views/matters/render"),
    ],
)
