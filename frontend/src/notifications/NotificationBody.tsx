 ...
 * `${client.first.last}` is whatever is in the record. Injecting that with
 * `dangerouslySetInnerHTML`, which is the obvious way to do this, means anyone
 * who can email the intake address can run script in a legal assistant's
...
export function bodySnippet(body: string | undefined, max = 150): string {
  ...
  try {
    ...
    // `<p>Discovery</p><p>Matter: …</p>` previews as "DiscoveryMatter:".
