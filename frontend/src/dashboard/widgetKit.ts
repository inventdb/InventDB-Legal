 ...
 *    dark theming for free — the app tokens inherit through the shadow
 *    boundary.
 *
 * SOAR's tokens are renamed to this app's here (`--fg` → `--text`, `--line` →
 * `--border`, and so on) and its font-size scale is written out, since the app
