...
/** "view →" goes to that type's own module, when the app has one for it. */
...
function TableView({ rows, money }: { rows: Row[]; money?: boolean }) {
  const cols = Object.keys(rows[0])
    ...
    .slice(0, 6);
  const moneyish = (c: string) => !!money && /fee|cost|amount|price|value|balance|total|rate|billed|collected|settlement/i.test(c);
  return (
