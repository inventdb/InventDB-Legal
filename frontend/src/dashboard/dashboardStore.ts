 ...
 * this app pins its namespace server-side — as the Files room does — so it lives
 * in `legal.dashboards` instead, keyed by user alone. That is the whole divergence:
...
async function read(userId: string | undefined): Promise<StoredDashboard | null> {
  try {
    const rows = await sql<StoredDashboard>(
      `SELECT _id, widgets FROM legal.${TYPE} WHERE dash_key = '${keyFor(userId)}' LIMIT 1`
    );
    return rows[0] ?? null;
  } catch {
