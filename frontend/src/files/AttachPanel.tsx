 ...
 * document: *which matter is this? whose file does this belong on?*
 *
 * Two ways to answer it, and they are different questions:
 *   • the record already exists — attach it (this is most of the time);
 *   • the file **is** the reason for a new record — a complaint served on a
 *     walk-in client is a matter waiting to be opened. Create it and attach in
 *     one go, so the document is on the file from the moment the file exists.
 */
 ...
 *
 * Pleadings, retainers and correspondence are overwhelmingly about a case, so
 * the matter path is the one offered by name rather than buried in a picker.
 */
const DEFAULT_NEW_TYPE = "matters";
    ...
    <div className="fx-attach">
      ...
      <div className="fx-attach-modes" role="tablist">
        ...
        <button
          ...
          onClick={() => setMode("new")}
        >
          <Briefcase size={13} /> Open a matter
        </button>
      </div>
