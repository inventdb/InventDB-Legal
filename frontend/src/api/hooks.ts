...
export const useCaseloadReport = reportHook<{
  by_status: { name: string; value: number }[];
  by_practice_area: { name: string; value: number }[];
  by_responsible_attorney: { name: string; value: number }[];
  total: number;
  open: number;
  open_rate: number;
}>("caseload", "caseload");
export const useTimeEntriesReport = reportHook<TimeEntriesReport>(
  "time-entries",
  "time-entries"
);
export const useArAging = reportHook<ArAgingReport>("ar-aging", "ar-aging");
export const useTrustCompliance = reportHook<TrustComplianceReport>(
  "trust-compliance",
  "trust-compliance"
);
...
/** Unbilled, billable time by matter — what is ready to invoice. */
...
/** The docket: open deadlines falling due inside `days`, plus anything overdue. */
export function useDeadlinesDue(days = 30) {
  return useQuery<{
    rows: DeadlineRow[];
    count: number;
    days: number;
    ...
    queryKey: ["report", "deadlines", days],
    queryFn: async () => {
      const { data } = await api.get("/reports/deadlines", { params: { days } });
      return data;
    },
  });
}
...
function useNotificationMutation<TArgs, TData>(fn: (args: TArgs) => Promise<TData>) {
  ...
  return useMutation<TData, unknown, TArgs>({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["notifications"] });
      // Answering releases a parked run, which then books the work and writes
...
export function useResolveNotification() {
  return useNotificationMutation(
    async ({
      ...
      // not nothing to a plan branching on `${decision.payload.approved}`.
 ...
 * unattached now belongs to a matter. `copy` adds a parent without removing the
 * first, for the order that really does resolve two motions.
 */
