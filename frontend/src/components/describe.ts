 ...
 * what an invoice is until it looks. This app already knows: `config/entities.ts`
 ...
 * column is how you write a record nothing else in the practice can read.
 */
 ...
 * arrives pre-filled with a plausible invented fee is worse than one with a
 * blank, because the blank is obviously unfinished and the invention is not.
 * And **no tools**: the agent behind this endpoint can query the practice, but
...
export function fillPrompt(
  ...
  return [
    `You are filling in the "New ${config.label}" form of a legal practice management app.`,
