...
const PROPOSED = [
  { kind: "kpi", title: "Overdue Rent", sql: "SELECT COUNT(*) AS v FROM legal.invoices", span: 3 },
  { kind: "bar", title: "Arrears by Region", sql: "SELECT region, COUNT(*) AS v FROM legal.matters GROUP BY region", span: 6 },
];
...
test.describe("Dashboard", () => {
  ...
  test("greets the operator and says where the figures come from", async ({ page }) => {
    await expect(page.getByText("Today · Legal")).toBeVisible();
  ...
  test("opens on the onboarding state when nothing has been built yet", async ({ page }) => {
    await expect(page.getByRole("heading", { name: /No widgets on the legal dashboard yet/ })).toBeVisible();
    await expect(page.getByRole("button", { name: /Suggest from my legal data/ })).toBeVisible();
  ...
  test.describe("adding a widget", () => {
    test("builds a one-query number and puts it on the grid", async ({ page }) => {
      ...
      await dialog.locator("#w-title").fill("Occupied Matters");
      await dialog.locator("#w-sql").fill("SELECT COUNT(*) AS v FROM legal.matters");
      await dialog.getByRole("button", { name: "Preview" }).click();
      await dialog.getByRole("button", { name: "Add widget" }).click();

      await expect(page.getByRole("heading", { name: "Occupied Matters" })).toBeVisible();
    ...
    test("the quick-add tab targets a module rather than a free-text type", async ({ page }) => {
      ...
      await expect(dialog.locator("#w-ns")).toHaveValue("legal");
      await dialog.locator("#w-formtype").selectOption("matters");
      // The preview is the ACTUAL card, so what you approve is what you get.
      await expect(dialog.getByRole("button", { name: /New Matter/ })).toBeVisible();
    });
  });
  ...
  test.describe("with a layout", () => {
    test.beforeEach(async ({ page }) => {
      ...
      const dialog = page.getByRole("dialog");
      await dialog.getByRole("tab", { name: "Simple (one query)" }).click();
      await dialog.locator("#w-title").fill("Matters by Status");
      await dialog.locator("#w-kind").selectOption("bar");
      await dialog
        .locator("textarea.we-sql")
        .fill("SELECT status, COUNT(*) AS v FROM legal.matters GROUP BY status");
      await dialog.getByRole("button", { name: "Add widget" }).click();
      await expect(page.getByRole("heading", { name: "Matters by Status" })).toBeVisible();
    });
    ...
    test("removes a widget", async ({ page }) => {
      ...
      await expect(page.getByRole("heading", { name: "Matters by Status" })).toHaveCount(0);
    ...
    test("asks before clearing the whole dashboard", async ({ page }) => {
      ...
      await expect(page.getByText(/Clear the legal dashboard\?/)).toBeVisible();
    ...
    test("rearranging never transforms a card mid-drag", async ({ page }) => {
      ...
      await dialog.locator("#w-sql").fill("SELECT COUNT(*) AS v FROM legal.clients");
      await dialog.getByRole("button", { name: "Add widget" }).click();
    ...
    test("a card can be moved in EITHER direction, and lands where it is dropped", async ({
      ...
      for (const [title, span] of [
        ...
        await d.locator("#w-sql").fill("SELECT COUNT(*) AS v FROM legal.matters");
