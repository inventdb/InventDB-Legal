...
test.describe("Settings", () => {
  ...
  test("reports the InventDB connection from /api/health", async ({ page }) => {
    ...
    await expect(connection).toContainText("legal");
  ...
  test("warns when the API cannot be reached", async ({ page }) => {
    ...
    await expect(page.locator(".alert.error")).toHaveText("Could not reach the Legal API.");
  });
  ...
  test("switches theme from the appearance card", async ({ page }) => {
    ...
    expect(await page.evaluate(() => localStorage.getItem("legal.theme"))).toBe("dark");
  ...
  test("signs out from the account card", async ({ page }) => {
    ...
    expect(await page.evaluate(() => localStorage.getItem("legal.user"))).toBeNull();
  });
});
