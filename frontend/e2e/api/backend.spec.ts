...
const ENTITIES = [
  "matters", "clients", "timekeepers", "invoices", "time_entries",
  "costs_and_disbursements", "payments_and_receipts", "court_calendar", "deadlines_and_sol", "lookups",
] as const;
...
test.describe("Health and shape", () => {
  test("health reports the instance it is pointed at", async ({ request }) => {
    ...
    expect(body.namespace).toBe("legal");
...
test.describe("Authentication", () => {
  ...
  test("a malformed Authorization header is refused", async ({ request }) => {
    for (const header of ["Bearer", "Token abc", "Bearer   ", "abc"]) {
      const res = await request.get("/api/matters", {
...
test.describe("Modules", () => {
  ...
  test("paging is honoured", async ({ request }) => {
    const res = await request.get("/api/matters?limit=1&offset=0", { headers: auth });
    expect(res.status()).toBe(200);
  ...
  test("a non-numeric limit is the caller's mistake, not a server fault", async ({ request }) => {
    ...
    for (const bad of ["abc", "1.5", "1e5", "10; DROP TABLE x"]) {
      const res = await request.get(`/api/matters?limit=${encodeURIComponent(bad)}`, {
        headers: auth,
      });
  ...
  test("a non-numeric offset is refused the same way", async ({ request }) => {
    const res = await request.get("/api/matters?offset=abc", { headers: auth });
    expect(res.status()).toBe(400);
  });
  ...
  test("a record round-trips: create, read, update, delete", async ({ request }) => {
    ...
    expect(String(client.client_id ?? "")).toMatch(/^CL-/);

    const read = await request.get(`/api/clients/${id}`, { headers: auth });
    expect(read.status()).toBe(200);

    const updated = await request.put(`/api/clients/${id}`, {
      headers: auth,
      data: { client_name: "E2E Holdings LLC" },
    });
    expect(updated.status()).toBe(200);

    const removed = await request.delete(`/api/clients/${id}`, { headers: auth });
...
test.describe("Metadata and SQL", () => {
  ...
  test("a SELECT passes through", async ({ request }) => {
    const res = await request.post("/api/meta/sql", {
      headers: auth,
      data: { sql: "SELECT * FROM legal.matters LIMIT 1" },
    });
    expect(res.status()).toBe(200);
  ...
  test("anything that is not a read is refused", async ({ request }) => {
    for (const sql of [
      ...
      "UPDATE legal.matters SET city = 'x'",
...
test.describe("Import", () => {
  test("a batch is relayed to the bulk endpoint", async ({ request }) => {
    const res = await request.post("/api/import/matters", {
      headers: auth,
      data: {
        rows: [
          { matter_caption: "Ortega v. Verdugo Hills" },
          { matter_caption: "Ortega v. Kang Brothers" },
        ],
      },
    });
    expect(res.status()).toBe(200);
  ...
  test("a malformed batch is refused", async ({ request }) => {
    for (const data of [{}, { rows: [] }, { rows: "nope" }, { rows: [1, 2] }]) {
      const res = await request.post("/api/import/matters", { headers: auth, data });
      expect(res.status(), JSON.stringify(data)).toBe(400);
    }
  });

  test("a dangerous type name never reaches InventDB", async ({ request }) => {
    const res = await request.post("/api/import/legal.matters", {
      headers: auth,
      data: { rows: [{ a: 1 }] },
    });
...
test.describe("Saved views", () => {
  test("a view is designed, saved, rendered, edited and removed", async ({ request }) => {
    const designed = await request.post("/api/views/matters/design", {
      headers: auth,
    ...
    const created = await request.post("/api/views/matters", {
      headers: auth,
    ...
    const rendered = await request.post("/api/views/matters/render", {
      headers: auth,
    ...
    const renamed = await request.put(`/api/views/matters/${id}`, {
      headers: auth,
      data: { name: "E2E cards renamed" },
    });
    expect(renamed.status()).toBeLessThan(300);

    const listed = await request.get("/api/views/matters", { headers: auth });
    expect(listed.status()).toBe(200);

    const removed = await request.delete(`/api/views/matters/${id}`, { headers: auth });
    expect(removed.status()).toBeLessThan(300);
  });

  test("a design with no instruction is refused", async ({ request }) => {
    const res = await request.post("/api/views/matters/design", {
      headers: auth,
      data: { instruction: "" },
    });
    expect(res.status()).toBe(400);
  ...
  test("rendering without a template is refused", async ({ request }) => {
    const res = await request.post("/api/views/matters/render", {
      headers: auth,
      data: { page: 0 },
    });
    expect(res.status()).toBe(400);
...
test.describe("Operations surfaces", () => {
  ...
  test("the read-only analyze SQL seam refuses a write", async ({ request }) => {
    const res = await request.post("/api/analyze/sql", {
      headers: auth,
      data: { sql: "DELETE FROM legal.matters" },
    });
  ...
  test("the analyze seam refuses a stacked statement", async ({ request }) => {
    const res = await request.post("/api/analyze/sql", {
      headers: auth,
      data: { sql: "SELECT 1; DROP TABLE legal.matters" },
    });
    expect(res.status()).toBe(400);
  });
});
