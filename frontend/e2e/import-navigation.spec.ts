...
function bigCsv(): Buffer {
  const lines = ["Matter ID,Client Name,Matter Caption"];
  for (let i = 0; i < 1200; i++) {
    lines.push(`MT-${9000 + i},Client ${i},Matter ${i}`);
  }
...
test("a finished import announces itself from another module", async ({ page }) => {
  ...
  console.log("toast on /clients:", JSON.stringify(await page.locator(".toast").innerText()));
});
