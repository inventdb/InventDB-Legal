...
test.describe("Analyze", () => {
  ...
  test("answers a question and shows its work", async ({ page }) => {
    await page.locator(".an-ask .an-composer-input").fill("What do we work on most?");
    await page.locator(".an-ask button[type=submit]").click();

    await expect(page.locator(".atl-answer")).toContainText(
      "Your three busiest practice areas are Personal Injury (155), Immigration (149) and Estate Planning (126)."
    );
    ...
    await expect(rail.filter({ hasText: "Grouping matters by city" })).toBeVisible();
  });
  ...
  test("the query behind the answer can be opened", async ({ page }) => {
    ...
    await expect(page.locator(".an-sql")).toContainText(
      "SELECT city, COUNT(*) AS cnt FROM legal.matters"
    );
  ...
  test("renders the chart the agent authored", async ({ page }) => {
    ...
    await expect(figure.locator(".an-figure-title")).toHaveText(
  ...
  test("offers the follow-ups the model proposed", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: "Which practice area bills the most per hour?" })
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: "✦ Email this breakdown to the partners" })
    ).toBeVisible();
  });
  ...
  test("a proposed change waits for approval", async ({ page }) => {
    ...
    await page
      ...
      .fill("Add the Barkley deposition transcript");
    await page.locator(".an-ask button[type=submit]").click();

    const card = page.locator(".an-card");
    await expect(card).toContainText("Create cost");
    await expect(card).toContainText("Barkley Court Reporters");
  ...
  test("a workflow the assistant builds arrives whole, step by step", async ({ page }) => {
    ...
    await page
      .locator(".an-ask .an-composer-input")
      .fill("Every month, email each client their statement.");
    await page.locator(".an-ask button[type=submit]").click();
    ...
    await expect(card).toContainText("Email clients");
  ...
  test("a thread survives leaving the room and coming back", async ({ page }) => {
    ...
    await expect(page.locator(".atl-answer")).toContainText("Personal Injury");
  });
  ...
  test("a follow-up runs the agent exactly once", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: "Which practice area bills the most per hour?" })
      .click();
