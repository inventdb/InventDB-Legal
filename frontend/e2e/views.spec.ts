...
/** Quote a label so it can be dropped into a RegExp as literal text. */
...
test.describe("View switcher", () => {
  ...
  test("starts with no saved views, so Manage has nothing to do", async ({ page }) => {
    ...
    await expect(page.locator(".vs-row")).toHaveCount(1); // just "All matters"
  ...
  test("saves the current search and makes it the current view", async ({ page, store }) => {
    await page.goto("/matters");
    const narrowed = matching(store.matters, "Pineda, Carmen");
    await page.getByPlaceholder("Search matters…").fill("Pineda, Carmen");
    await expect(rows(page)).toHaveCount(narrowed);

    await saveCurrentAs(page, "Pineda, Carmen stock");

    await expect(page.locator(trigger)).toHaveText(/Pineda, Carmen stock/);
    await expect(rows(page)).toHaveCount(narrowed);
  });
  ...
  test("switching back to All clears the view's filter", async ({ page, store }) => {
    ...
    await expect(page.locator(trigger)).toHaveText(/All matters/);
    await expect(page.getByPlaceholder("Search matters…")).toHaveValue("");
    await expect(rows(page)).toHaveCount(store.matters.length);
  });
  ...
  test("re-opening a view restores its search term", async ({ page }) => {
    ...
    await page.locator(trigger).click();
    await page.getByRole("button", { name: "Pineda, Carmen stock" }).click();

    await expect(page.getByPlaceholder("Search matters…")).toHaveValue("Pineda, Carmen");
  });
  ...
  test("filters the palette as you type", async ({ page }) => {
    ...
    await page.getByPlaceholder("Search matters…").fill("Pineda, Carmen");
    await saveCurrentAs(page, "Pineda, Carmen stock");
    await page.getByPlaceholder("Search matters…").fill("");
    await saveCurrentAs(page, "Everything");

    await page.locator(trigger).click();
    await page.getByPlaceholder("Search views…").fill("pineda");

    await expect(page.locator(".vs-row")).toHaveCount(1);
    await expect(row(page, "Pineda, Carmen stock")).toBeVisible();
  });
  ...
  test("a view is scoped to its own module", async ({ page }) => {
    ...
    await page.goto("/timekeepers");
    await page.locator(trigger).click();

    await expect(page.locator(".vs-row")).toHaveCount(1);
    await expect(page.locator(palette)).not.toContainText("Pineda, Carmen stock");
  });
});
...
test.describe("Manage", () => {
  ...
  test("deleting the current view falls back to the full list", async ({ page, store }) => {
    await page.goto("/matters");
    await page.getByPlaceholder("Search matters…").fill("Pineda, Carmen");
    await expect(rows(page)).toHaveCount(matching(store.matters, "Pineda, Carmen"));
    await saveCurrentAs(page, "Pineda, Carmen stock");
    await expect(page.locator(trigger)).toHaveText(/Pineda, Carmen stock/);
...
test.describe("New view — the designer", () => {
  ...
  test("saving turns the design into a view that renders in the list", async ({ page }) => {
    ...
    await page.getByLabel("Describe the view").fill("a card per property");
    await page.getByRole("button", { name: "Design view" }).click();
    await page.locator("#designed-view-name").fill("Matter cards");
    await page.getByRole("button", { name: "Save view" }).click();

    await expect(page.locator(designer)).toBeHidden();
    await expect(page.locator(trigger)).toHaveText(/Matter cards/);
