...
async function resolveBindings(
  ...
  const list = await callApi(page, "GET", "/api/matters?limit=1", null);
...
test.describe("mocked API returns the same fields as the real backend", () => {
  ...
  for (const endpoint of contract.endpoints) {
    ...
    test(`${endpoint.key} exposes every top-level field`, async ({ page }) => {
      ...
      const isEnvelope = !["matter-detail", "matter-create", "matter-update"].includes(
        endpoint.key
      );
