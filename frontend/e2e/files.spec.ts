...
test.describe("Files", () => {
  ...
  test("groups the tree by record type, then by folder", async ({ page }) => {
    // Type first, not path first: "2026" under invoices and "2026" under
    // court_calendar are different folders and must not merge into one node.
    await expect(tree(page).locator(".fx-node--type")).toHaveCount(2);
    await expect(tree(page).getByRole("button", { name: "invoices", exact: true })).toBeVisible();
    await expect(tree(page).getByRole("button", { name: "court_calendar", exact: true })).toBeVisible();
  });

  test("rolls a folder's count up into its type", async ({ page }) => {
    // invoices holds one unfoldered file plus one in 2026.
    const invoices = tree(page).locator(".fx-node--type").filter({ hasText: "invoices" });
    await expect(invoices.locator(".fx-node-count")).toHaveText("2");
  });

  test("nests a deep folder under its parent rather than flattening it", async ({ page }) => {
    await tree(page).getByRole("button", { name: "Expand court_calendar" }).click();
  ...
  test("a deep link opens straight into a folder", async ({ page }) => {
    await page.goto("/files?type=court_calendar&folder=2026/photos");
  ...
  test("every row says where the file lives", async ({ page }) => {
    ...
    await expect(row).toContainText("invoices › 2026");
  });
  ...
  test("opening a file shows what it is and where it lives", async ({ page }) => {
    ...
    await expect(modal).toContainText("invoices / inv-1");
  ...
  test("a file on a record says which record, and links to it", async ({ page }) => {
    ...
    await expect(dialog(page).locator(".fx-attached")).toContainText("invoices / inv-1");
  ...
  test("an unattached file says so and offers to place it", async ({ page }) => {
    // rent-schedule.xlsx came in through the drive, so it sits in the invoices
    ...
    await expect(dialog(page).getByRole("tab", { name: "Open a matter" })).toBeVisible();
  });
  ...
  test("attaching puts the file on the record you pick", async ({ page }) => {
    ...
    await rows(page).filter({ hasText: "rent-schedule.xlsx" }).click();
    await dialog(page).getByRole("tab", { name: "Details" }).click();
    await dialog(page).locator("#fx-attach-type").selectOption("time_entries");
    await dialog(page).locator("#fx-attach-record").selectOption("te-1");
    await dialog(page).getByRole("button", { name: "Attach" }).click();

    await expect(page.locator(".toast")).toContainText("Now on time_entries / te-1");
  ...
  test("opens a matter from a file and attaches it in one go", async ({ page }) => {
    // A complaint served on a walk-in client is a matter waiting to be opened
    // — creating the matter and filing the document should not be two errands.
    const posted: string[] = [];
    ...
    await rows(page).filter({ hasText: "rent-schedule.xlsx" }).click();
    await dialog(page).getByRole("tab", { name: "Details" }).click();
    await dialog(page).getByRole("tab", { name: "Open a matter" }).click();

    // The matter form is the app's own — same fields as the module.
    await dialog(page).locator("#f-matter_caption").fill("Ortega v. Verdugo Hills");
    await dialog(page).getByRole("button", { name: "Save" }).click();

    await expect(page.locator(".toast")).toContainText("attached this file to it");
    expect(posted).toHaveLength(1);
    expect(attached).toMatchObject({ type: "matters", mode: "move" });
  });
  ...
  test("deleting a type asks before removing anything", async ({ page }) => {
    ...
    await expect(page.getByText("Delete all 2 file(s) on invoices?")).toBeVisible();
  ...
  test("deleting the folder you are standing in drops back to All files", async ({ page }) => {
    ...
    await page.goto("/files?type=invoices&folder=2026");
    await expect(rows(page)).toHaveCount(1);
