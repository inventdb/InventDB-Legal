...
test.describe("API failure handling", () => {
  test("shows the server's message when a list call fails", async ({ page }) => {
    await page.route("**/api/matters*", (route) =>
      route.fulfill({
        ...
        body: JSON.stringify({ error: "Namespace legal is not reachable" }),
      })
    );
    await page.goto("/matters");

    await expect(page.locator(".alert.error")).toHaveText("Namespace legal is not reachable");
  });
  ...
  test("renders a network-level failure rather than an empty table", async ({ page }) => {
    await page.route("**/api/costs_and_disbursements*", (route) => route.abort("connectionrefused"));
  ...
  test("logs the user out when any call returns 401", async ({ page }) => {
    ...
    expect(await page.evaluate(() => localStorage.getItem("legal.token"))).toBeNull();
  });

  test("a 401 during a write also ends the session", async ({ page }) => {
    await page.goto("/matters");
    await page.getByRole("button", { name: "New Matter" }).click();
    await page.locator("#f-matter_caption").fill("Doomed v. Nobody");
  ...
  test("recovers when the API comes back", async ({ page }) => {
    let failing = true;
    await page.route("**/api/timekeepers*", async (route) => {
      if (failing) {
        return route.fulfill({
          status: 503,
          contentType: "application/json",
