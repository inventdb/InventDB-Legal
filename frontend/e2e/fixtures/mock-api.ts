...
export async function installMockApi(
  ...
  // What `SELECT ... FROM legal.<type>` answers on this dashboard. A spec that
  ...
  await page.route(API_ROUTE, async (route) => {
    ...
    if (head === "reports") {
      ...
      if (rest[0] === "widgets") {
        ...
        if (rest[1] === "design") {
          return json(route, {
            ...
            base_sql: String(body.base_sql ?? "SELECT * FROM legal.matters"),
          });
        }
      ...
      if (rest[0] === "caseload") return json(route, CASELOAD);
      if (rest[0] === "time-entries") return json(route, TIME_ENTRIES_REPORT);
      if (rest[0] === "cashflow") return json(route, { cashflow: DASHBOARD_CHARTS.cashflow });
      if (rest[0] === "wip") return json(route, WIP);
      if (rest[0] === "deadlines") return json(route, DEADLINES_DUE);
      if (rest[0] === "ar-aging") return json(route, AR_AGING);
      if (rest[0] === "trust-compliance") return json(route, TRUST_COMPLIANCE);
      return json(route, { error: "Not found" }, 404);
    }
    ...
    if (head === "files") {
      ...
      if (first === "attach" && method === "POST") {
        ...
        return json(route, {
          mode: body.mode ?? "move",
          parents: [
            { namespace: "legal", typeName: String(body.type ?? ""), recordId: String(body.record_id ?? "") },
          ],
        });
      }
            ...
            plan: [
              {
                ...
                sql: "SELECT _id FROM legal.invoices WHERE status = 'overdue'",
              },
            ],
          },
    ...
    if (head === "analyze") {
      ...
      if (rest[0] === "records") {
        return json(route, { ok: true, recordId: nextId("costs_and_disbursements"), type: rest[1] });
      }
    ...
    if (head === "meta") {
      ...
      if (rest[0] === "sql") {
        ...
        if (/from\s+legal\.dashboards/i.test(statement)) {
