 ...
 *
 * This goes through the real form rather than writing `legal.token` directly, so
 ...
 *
 * It also pins `legal.theme` to light, which keeps the theme-dependent
...
setup("authenticate", async ({ page }) => {
  ...
  await page.evaluate(() => localStorage.setItem("legal.theme", "light"));

  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("legal.token")))
    .not.toBeNull();
