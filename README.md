...
**A complete legal practice management system — powered by [InventDB SOAR](https://www.inventdb.com).**

Matters, clients, time, costs, billing, trust accounting, the docket, AI analysis,
reports and automations in one fast, responsive workspace.
...
│  dashboard, CRUD │ ◄───── │  proxy · aggregate    │ ◄───── │ namespace "legal"│
└──────────────────┘  JSON  └───────────────────────┘  JSON  └──────────────────┘
```

Three things follow from this shape:

- **InventDB is the database.** All data lives in one namespace (`legal`), created automatically on first write. There is no second datastore to install, migrate or back up.
...
| Module | Tracks |
|---|---|
| 💼 **Matters** | Practice area, matter type, stage, court, case number, judge, fee model, limitations date, opposing party & counsel |
| 👤 **Clients** | Individuals and entities, contact, language, referral source, conflict check, engagement letter, portal access |
| 👥 **Timekeepers** | Role, bar number, admission date, standard and cost rates, targets, MCLE compliance |

### Time & billing

| Module | Tracks |
|---|---|
| ⏱️ **Time Entries** | UTBMS phase/task/activity codes, narrative, hours, rates, billable & billed state, write-downs |
| 🧾 **Costs & Disbursements** | UTBMS expense codes, vendor, amount, advanced/recoverable, what was billed on |
| 📄 **Invoices** | Period, fees, costs, write-downs, applied-from-trust, balance due, status and aging bucket |

### Money

| Module | Tracks |
|---|---|
| 💰 **Payments & Receipts** | Amount, method, payor, which invoice, and whether it landed in operating or trust |
| 🏛️ **Trust Ledger (CTA)** | Every deposit, transfer and disbursement, with the running matter and account balances |
| 🤝 **Settlements & Liens** | Gross settlement, fee basis and rate, costs reimbursed, liens claimed vs paid, net to client |

### Calendar, docket & intake
...
- Homeless files can be attached, or open a matter to hold them
...
`<entity>` is one of `matters`, `clients`, `timekeepers`, `time_entries`,
`costs_and_disbursements`, `invoices`, `payments_and_receipts`,
`trust_ledger_cta`, `court_calendar`, `deadlines_and_sol`, `intake_and_leads`,
`settlements_and_liens`, `lookups`.

### Reports

| Method | Path | Purpose |
|---|---|---|
| `GET` | `/api/reports/{pnl,cashflow,wip,deadlines,caseload,time-entries,ar-aging,trust-compliance}` | Computed by InventDB SQL |
...
│   │   │                      #   Files · Import · Settings · Login · EntityListPage
...
│   │   └── api/ auth/ theme/ utils/ styles/ views/ dashboard/
│   ├── e2e/                   # Playwright specs
│   └── vite.config.ts · package.json
├── contract/                  # Generated shape of every /api response
└── CA_Litigation_Practice_Dataset.xlsx   # The dataset the `legal` namespace holds
```
...
| A chart shows one huge bar | An aggregate grew a function call in its `WHERE` beside a `GROUP BY`. See the section above |
...
- SQL identifiers are validated against a strict allow-list; all values are escaped, and search terms have their `LIKE` wildcards escaped too.
